  ...
  <div v-if="view">
    <div class="preview-container">
      ...
      <div class="image-container">
        ...
        <UserAvatar class="avatar max-h" :img="state?.photos" :sex="user?.sex" />
...
export default defineComponent({
  ...
  components: {UserAvatar, InputImages, InfoBlock, GenderIcon, ButtonPrimary},
  ...
  setup() {
   ...
   date of birth returned from the backend via the api endpoint */
    ...
    function updateImages(imagesNew) {
      state.value.photos = imagesNew
      console.log("state.photos: ", state.value.photos);
    }
    ...
    //
    //     state.value.photos = rawImg
    //
    //     // change main photo
    //     images.value[0] = rawImg;
    //
    //   }
    ...
    onBeforeMount(()=>{
      if(state.value.photos) {
        images.value.push(state.value.photos)
      }
    })

    return {user, state, v$, updateImages, submitForm, years, days, edition, view, online, yearsOld, images
      // uploadAvatar
    };
  },
})
