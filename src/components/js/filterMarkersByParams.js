export function filterMarkersByParams(markers, params) {
    ...
    // if (params.checkboxes['other']['only my events']) {
    //     filteredMarkers = filteredMarkers.filter(function (marker) {
    //         return marker.ImAParticipant
    //     })
    // }
    ...
    // if (params.checkboxes['other']['without alcohol']) {
    //     filteredMarkers = filteredMarkers.filter(function (marker) {
    //         return !marker.withoutAlcohol
    //     })
    // }
            ...
            searchQuery.trim().split(' ').forEach((value) => {
                if(value !== '') {
                    ...
                    if(marker.type.toLowerCase().indexOf(value) !== -1)
                        go = true
                    // if(marker.music.toLowerCase().indexOf(value) !== -1)
                    //     go = true
                }
            })
