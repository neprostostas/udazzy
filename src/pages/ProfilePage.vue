...
<!--      <UserAvatar class="avatar" :img="user?.photo" :sex="user?.sex" />-->
      <UserAvatar class="avatar" :img="user?.photos" :sex="user?.sex" />
...
export default defineComponent({
  ...
  setup(props) {
    ...
    /* import instance of loader from loaderStore */
    const loader = loaderStore()

    /* constant that show is user online */
    ...
    /* a function that checks whether the user rendered on
       the profile page is a logged-in user or another person */
    const isItMe = computed(() => {
      return !props.uri || props.uri === JSON.parse(localStorage.getItem('user_data')).uri;
    })

    /* watch function that re-render profile
       page if logged-in user has been changed */
    watch(isItMe, () => {
    ...
    /* get user data when components mount in DOM-tree */
    onMounted(() => {
      profile.getUserData(props.uri, isItMe, user);
      profile.getMeData(me);
    })

     /* if user name, age or date of birthday is empty by any reason - open signup page */
    watchEffect(()=>{
