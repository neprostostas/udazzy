    ...
    <div class="wrapper">
      <div class="item">
        ...
        >
          ...
          <!--          <gmap-custom-marker :z-index="-999999" :marker="venuesFiltered[0].position"></gmap-custom-marker>-->
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function mapIsReady() {
      if (timeout)
          // if(mapRef.value.map.getProjection() !== null && mapRef.value.map.getProjection() !== undefined)
        return true;
      else return false;
    }
    ...
    function markersCluster(venues = venuesFiltered.value) {
      venues.forEach((venue) => {
        ...
        // TODO коли Олег зробить групування по плейсах
        // if (mapIsReady()) {
        //   markers.forEach((comparableMarker) => {
        //     if (marker !== comparableMarker) {
        //       let distance = spaceBetweenMarkers(mapRef.value.map, marker, comparableMarker)
        //       if (distance < 70) {
        //         if (marker.priority < comparableMarker.priority) {
        //           marker.icon.scaledSize = {width: 20, height: 20}
        //           marker.icon.anchor = {x: 10, y: 10}
        //           marker.label.className = 'marker-label-mini'
        //         } else {
        //           comparableMarker.icon.scaledSize = {width: 20, height: 20}
        //           comparableMarker.icon.anchor = {x: 10, y: 10}
        //           comparableMarker.label.className = 'marker-label-mini'
        //         }
        //       }
        //     }
        //   })
